...
ShinoUploads = [Ultimate_Speedrun, GoingUP, SandySanctuary]
...
TKUploads = [Phoenix, Cistus_Albidus, SeriousMonsterMystery, Regain_Peace]
...
ShyGuy64Uploads = [Wrath_of_Leviathan, AdventuresEnd]
ShyGuy64Stats = [ShyGuy64Uploads, "ShyGuy64"]

Ev_BrakUploads = [Delirium_Trigger, Katz_Kradle, MorphasManor]
...
YoshiUploads = [Glacial, GlacialEclipse]
...
NezuUploads = [LostCollection]
NezuStats = [NezuUploads, "Nezu"]

HaeflyUploads = [RunnersHigh]
HaeflyStats = [HaeflyUploads, "Haefly"]

IFILYびつきーUploads = [Sanduleak]
IFILYびつきーStats = [IFILYびつきーUploads, "IFILYびつきー"]

FirimUploads = [PainForPlesure]
FirimStats = [FirimUploads, "Firim"]

LeoX2Uplopads = [Legacy]
LeoX2Stats = [LeoX2Uplopads, "LeoX2"]

ZorenUploads = [SapphireSanctum]
ZorenStats = [ZorenUploads, "Zoren"]

UploadsArray = [IkglStats, PeteStats, AyaStats, ShinoStats, FailStats, LunaStats, HayakuStats, ChocoStats, AndrewStats, MokitanStats, RicoStats, BenjaStats, SKG_D3ふぁんStats, わぎゃっとStats, AliceStats, schwarzeStats, KatzStats, JonasHamStats, TKstats, MotchStats, CasterStats, WreferiiiStats, りようま_ようつべStats, N_IああいStats, EthanStats, アルチェStats, MT_コンドリアStats, AXL_LN_JGMStats, ItzyStats, MurmeliStats, DogumaStats, Z7Stats, ParasolStats, ばんりStats, JCStats, ShyGuy64Stats, Ev_BrakStats, Zone_ののむらあStats, DanteStats, KingBooStats, YoshiStats, 叶音Stats, Ln_XCStats, NezuStats, HaeflyStats, IFILYびつきーStats, FirimStats, LeoX2Uplopads, ZorenStats]


function PlayerUploads() {
  index = 0
  PlayerID = 0
  while (PlayerID < UploadsArray.length) {
    while (index < UploadsArray[PlayerID][0].length) {
      ListPage.getRange(UploadsArray[PlayerID][0][index], 5).setValue(UploadsArray[PlayerID][1]) 
      index++
    }
    index -= index
    PlayerID++
...
console.log("Finished the player uploads execution")
